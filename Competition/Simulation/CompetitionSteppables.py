    ...
    def step(self, mcs):
        ...
        if mcs > relaxtime:
            for cell in self.cellList:
                ...
                if cell.type in [1, 3, 5, 7]:  # Growth for specific cell types (wild type)
                    ...
                    tvadded += round(random.normalvariate(growthratewt, 2.5)) * \
                        np.exp(-(CI / mcs) * ((cell.volume - cell.targetVolume) ** 2))
                elif cell.type in [2, 4, 6, 8]:  # Growth for other specific cell types (scrambled)
...
class MitosisSteppableAdder(MitosisSteppableBase):  
    ...
    def updateAttributes(self):
        ...
        # Update cell types to reflect progression through the cell cycle
    ...
    def step(self, mcs):
        ...
        if mcs > relaxtime:  # Skip apoptosis calculations during the relaxation period
            for cell in self.cellList:  # Iterate over all cells in the simulation
                if cell.type != 9 and cell.type != 10:  # Skip already apoptotic cells
                    totdens = 0
                ...
                if cell.type in [2, 4, 6, 8]:  # Record for scrambled cells
                    LocalDensity.append([cell.id, cell.type, time, totdens])
                elif cell.type in [1, 3, 5, 7]:  # Record for wild-type cells
                    LocalDensity.append([cell.id, cell.type, time, totdens])
    ...
    def step(self, mcs):
        ...
        if mcs > relaxtime:  # Skip apoptosis calculations during the relaxation period
            for cell in self.cellList:  # Iterate over all cells in the simulation
                ...
                typecell = 2 if cell.type in [2, 4, 6, 8] else 1
                ...
                if cell.type not in [9, 10]:  # Skip already apoptotic cells
...
class neighbourdata(SteppableBasePy):
    ...
    def step(self, mcs):
        ...
        for cell in self.cellList:
            if cell.type in [1, 3, 5, 7]:  # Wild-type cells
                cellcount_wt += 1
            elif cell.type in [2, 4, 6, 8]:  # Scrambled cells
                cellcount_scrb += 1
...
class tracking(SteppableBasePy):   
    ...
    def step(self, mcs): 
        ...
        for cell in self.cellList:
            ...
            # State 1 indicates the cell is apoptotic (type 9 or 10)
